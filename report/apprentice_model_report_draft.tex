...
In the teacher labelling stage, a teacher model $T$ produces a label $\tilde{y}_i$ and confidence scores $p_T(y=0|x_i)$ and $p_T(y=1|x_i)$. 
The distilled student will then learn to imitate the teacher distribution instead of only relying on the original hard labels.

For a student model $S$, the distillation goal can be written as a combination of supervised classification loss and teacher matching loss:
\begin{equation}
...
The supervised student and teacher labelling components have been implemented and the final distilled training stage uses the generated teacher labels and probabilities.
