...
I implemented soft label temperature distillation from the local BERT-base teacher into BERT-tiny on the natural stratified split.
...
\end{equation}
At the default threshold of 0.50, the soft label distilled BERT-tiny model reaches test F1 0.565 on the natural split.
...
If we compare the BERT-base teacher's hard predictions with soft labels, the distilled student got test F1 0.679 and agreement 0.928.
...
The aggregate counts show that soft label distillation mostly improves BERT-tiny by reducing false positives.
...
For the natural split comparison, I also benchmarked all final models on CPU over the same 3,000 test examples with one repeat.
